...
const ContentBlocks = ({ contentBlock }: contentBlockProps) => {
  ...
  return (
    <div className={"flex gap-4 my-8 h-60"}>
      {contentBlock.map((block) => {
        if (block.type === "video") {
          const thumbnailUrl = getYouTubeThumbnail(block.config.url);

          return (
            <li key={block.title} className={"relative list-none"}>
              {thumbnailUrl && (
                <Image
                  src={thumbnailUrl}
                  alt={block.title}
              ...
              <Image
                ...
                width={33.84}
                height={40}
                className={"absolute right-48 top-24 cursor-pointer"}
